...
class ThingSegmentorWidget(SaveableWidget, metaclass=QtABCMeta):
    ...
    def setData(self, img: np.ndarray, things: List[LabeledROI]=[]):
        self._img = img
        self._things = things
        self._setZ(0)
    ...
    def getData(self) -> List[LabeledROI]:
        return self._things
...
class ThingsSegmentorWindow(MainWindow):
    ...
    def _toPyQtGraph(self, things: List[LabeledROI]):
        '''
    ...
    def _fromPyQtGraph(self, things: List[LabeledROI]):
        ''' 
        Transform things appropriately for rendering in pyqtgraph's weird orientation defaults
        '''
        offset = np.array([0, self._ylen-self._xlen])
